...
> 以下为同一次对话生成的实际截图（示例：五年级信息科技「冒泡排序体验」单元）

### 教学工具页面

多步骤引导式交互：填写信息 → 动手模拟排序 → 知识问答，完成后自动提交数据到 QuickForm。

![教学工具页面截图](assets/screenshots/teaching-tool.png)

### 数据可视化大屏

顶部核心指标实时统计（完成人数、满分人数、平均得分、最快通关）+ 各题正确率柱状图 + 得分排行榜，每 5 秒自动刷新，适合课堂大屏投影展示。

![数据大屏截图](assets/screenshots/dashboard.png)
